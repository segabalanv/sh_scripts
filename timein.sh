...
# Tme zone database ref: http://www.twimsun.com/tz/tz-link.htm
...
if [ $# -eq 0 ] ; then
  ...
  ( echo "All nown time zones and regions defined on this system:"
    cd $zonedir
  ...
  #   Start by just counting matches.
  ...
  if [ "$matchcnt" -gt 0 ] ; then
    ...
    mixedregion="$(echo ${region%${region#?}} \
                | tr '[[:lower:]]' '[[:upper:]]')"
    mrappend="$(echo ${region#?} | tr '[[:upper:]]' '[[:lower:]]')"
    mixedregion="$mixedregion$mrappend"
