...
#   the thrash directories tamed. Otherwise, nothing will ever actually
...
do
  case $opt in
    f ) exec $realrm "$@"     ;;  # exec lets us exir this script directly.
    * ) flags="$flags -$opt"  ;;  # Other flags are from rm, not us
  esac
done
shift $(( $OPTIND - 1 ))
...
# Make suere that the $archivedir exists.
